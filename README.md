...
Production target: https://apps.matteob.dev/projects/webtunes
...
- **Suggested Imports** batch-map Top-100 (or recent-library) artist/title tags
  to MusicBrainz IDs through ListenBrainz, discover a balanced candidate set,
  and keep 20 private, playable imports staged per user. Slow AcoustID audio
  fingerprinting is only a bounded fallback for missing tags or mapping misses.
  Accept promotes a track into the library; reject deletes its objects and
  starts a 90-day cooldown. Suggested and manual downloads share one
  advisory-locked yt-dlp lane, with manual imports prioritized.
...
Suggested Imports requires `LISTENBRAINZ_TOKEN`; the refill worker resolves only
the user's current seed set in batches, so there is no full-library identity
backfill to run. `scripts/recognize-missing-metadata.mjs` remains a targeted
backfill for tracks actually missing artist, album, or art.
