...
readonly APP_ROOT="/home/debian/WebTunes"
...
for command in flock git install pgrep readlink runuser systemctl; do
...
[[ ${SCRIPT_ROOT} == "${APP_ROOT}" ]] || die \
  "This production-only script must run from ${APP_ROOT}; got ${SCRIPT_ROOT}."
[[ $(stat -c '%U' "${APP_ROOT}") == "${APP_USER}" ]] || die \
  "${APP_ROOT} must be owned by ${APP_USER}."
...
install -o root -g root -m 0644 \
  "${APP_ROOT}/deploy/webtunes-backup.service" \
